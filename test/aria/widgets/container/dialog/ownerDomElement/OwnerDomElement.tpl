...
{Template {
   $classpath : "test.aria.widgets.container.dialog.ownerDomElement.OwnerDomElement",
   $css : ["test.aria.widgets.container.dialog.ownerDomElement.OwnerDomElementStyle"],
   $hasScript : true
}}
    ...
    {macro dialogButton(name, containerId, modal)}
        ...
        {@aria:Dialog {
            ...
            maximizable: true,
            modal: modal,
            ownerDomElementId: containerId ? $getId(containerId) : null,
            bind: {
                center: {
                    to: name+"Center",
                    inside: data
